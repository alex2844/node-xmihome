...
Регистрирует классы устройств из пакета `xmihome-devices`, делая их доступными для `findModel` и `create`.
...
Создает экземпляр нужного класса устройства (`Device` или его подкласс) на основе модели. Если для модели нет специального класса, пытается загрузить спецификацию из облака MiOT.
...
Генерирует уникальный строковый идентификатор для экземпляра устройства на основе его конфигурации.
...
Определяет наиболее вероятный тип подключения (`miio`, `bluetooth`, `cloud`) на основе доступных полей в конфигурации устройства.
...
Вызывает определенное действие на устройстве. Используется для операций, которые не вписываются в модель get/set, например, запуск цикла уборки на пылесосе.
...
| `params` | `any[]` | (Опционально) Массив параметров для действия. |

**Возвращает:**

- `Promise<any>`: Промис, который разрешается результатом выполнения действия.
