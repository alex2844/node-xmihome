...
- **API-документация (`packages/node/`):**
  ...
  - **ДЛЯ КЛАССОВ, НАСЛЕДУЮЩИХ EventEmitter:**
    - **ОБЯЗАТЕЛЬНО** добавляй секцию **"События"** после секции "Методы".
    - Найди все вызовы `this.emit(...)` в коде класса.
    - Для каждого уникального события задокументируй:
      - Название события (первый аргумент `emit`)
      - Когда происходит (триггер)
      - Структуру данных payload (последующие аргументы `emit`)
      - Пример использования с `on()` или `once()`
