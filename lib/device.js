...
* TODO:
* вынести auth
* если вырубить чайник, затем вызвать destroy, то процесс не завершится, добавить device.isDestroy и device.reconnectTimer
*/
...
const RECONNECT_MAX_ATTEMPTS_SHORT = 5;
const RECONNECT_MAX_ATTEMPTS_LONG = 3;
...
export class Device extends EventEmitter {
	...
	/**
	 * Флаг, указывающий, что устройство в данный момент находится в процессе (пере)подключения.
	 * Предотвращает множественные одновременные попытки переподключения.
	 * @type {boolean}
	 */
	isConnecting = false;
	...
	/**
	 * Промис, представляющий текущую активную операцию подключения/переподключения.
	 * Используется для предотвращения множественных одновременных вызовов connect()
	 * и для того,  чтобы другие операции могли дождаться завершения подключения.
	 * Обнуляется после успешного подключения или ошибки.
	 * @private
		...
		this.#connectionPromise = (async () => {
			...
			try {
				if (connectionType === 'miio') {
					...
					const id = Device.getDeviceId(this.proxy);
					this.device = device;
	...
	async disconnect() {
		if (this.isConnecting) {
			this.client.log('debug', `Device "${this.getName()}" disconnect attempt during connection, waiting for connection to complete/fail.`);
			try {
	...
	async #handleExternalDisconnect(reason) {
		...
		while (true) {
			...
			// if ((currentAttempt === 1) || this.isConnecting) {
			...
			try {
				...
				// if ((currentAttempt === maxAttemptsInPhase) && (!isShortAttemptPhase || (RECONNECT_MAX_ATTEMPTS_LONG === 0)))
				// 	break;
