...
export default class extends Device {
	...
	/**
	 * Устанавливает соединение с устройством и выполняет аутентификацию.
	 * @async
	 * @override
	 */
	async connect() {
		...
		try {
			...
			this.client.log('error', `Kettle ${this.config.mac}: Bluetooth authentication failed:`, error);
			await this.disconnect().catch(e => this.client.log('warn', `Kettle ${this.config.mac}: Error during disconnect after failed auth:`, e));
			throw error;
		}
	};
