...
class Bluetooth extends EventEmitter {
	...
	async getDevice(mac) {
		...
		try {
			...
			this.client.log('info', `Device ${mac} not found directly, waiting for discovery...`);
			const config = await new Promise(resolve => {
				...
				if (!isDiscovering)
					this.startDiscovery();
				this.once(`available:dev_${id}`, config => {
					if (!isDiscovering)
						this.stopDiscovery();
					resolve(config);
				});
	...
	async startDiscovery(filters) {
		...
		try {
			...
			this.client.log('error', 'Failed to start Bluetooth discovery:', error);
			throw error;
		}
	};
	...
	async stopDiscovery() {
		...
		try {
			...
			this.client.log('error', 'Failed to stop Bluetooth discovery:', error);
			throw error;
		}
	};
